  ...
  return (
    <div className='app-container'>
      <div className='header-container'>
        <div className='header-left-container'><p>CS 121 / IN4MATX 141</p></div>
        <div className='header-right-container'><p>Search Engine</p></div>
      </div>
            ...
            {results.length > 0 &&
              results.map((item, index) => (
                <div key={index} className='output-list-item'>
                  ...
                  <button onClick={() => handleSummarize(item.url)} className='summarize-button'>
                    Summarize
                  </button>
                  {summaries[item.url] && <p className="summary-container">{summaries[item.url]}</p>}
                </div>
              ))}
          </div>
        </div>
